...
public class Test_multi_cnrt extends Thread{
    private int number;
    private CnrtNet net;
...
//                bi = resize(img_name, picSize, picSize);
    ...
    public Test_multi_cnrt(CnrtNet net, int number, CountDownLatch latch, List<String> fileResult, int picSize, long new_sess, int startIndex, int endIndex)
    {
        this.net = net;
        this.number = number;
        this.latch = latch;
...
//                float[][] new_output = net.forwardResizeRgb(rgb, bi.getWidth(), bi.getHeight(), picSize, 0, new_sess);
                float[][] new_output = net.forwardExpandImgRgb(rgb, bi.getWidth(), bi.getHeight(), picSize, 0, new_sess);
                long endTime = System.currentTimeMillis();
    ...
    public static void main(String args[]) throws InterruptedException {
        ...
        for (int i = 0; i < num; i++) {
            ...
            Thread thread = new Test_multi_cnrt(net, i, latch, fileResult, picSize, new_sess, start, end);
            thread.start();
        }
